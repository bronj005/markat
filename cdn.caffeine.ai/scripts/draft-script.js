(function() {
    ...
    function attachBadgeUpdateListeners() {
        ...
        window.addEventListener('scroll', scheduleBadgeUpdate, {
            passive: true
        });
        ...
        this.rebuildSelection = function(ids) {
            ...
            requestAnimationFrame(() => {
                ...
                if (selectedElementIds.length > 0) {
                    ...
                    toParent({
                        type: 'draft-editor:status',
                        payload: {
                            status: this.editorState.type
                        }
                    });
                }
            ...
            switch (type) {
                case 'draft-editor:ready':
                    ...
                    toParent({
                        type: 'draft-editor:status',
                        payload: {
                            status: selfRef.editorState.type
                        }
                    });
                    break;
                ...
                case 'draft-editor:remove-selection':
                    if (payload ? .element) {
                        toParent({
                            type: 'draft-editor:status',
                            payload: {
                                status: `removing ${payload.element}`
                            }
                        });
